...
async function setArtileList() {
  const res = await axios({
    ...
    params: queryObj,//这个地方相当于用上面的模板 axios获取到数据存到res里面
  })
  ...
  <tr>
        <td>
            <img src="${
              item.cover.type === 0
                ? `https://img2.baidu.com/it/u=2640406343,1419332367&amp;fm=253&amp;
            fmt=auto&amp;app=138&amp;f=JPEG?w=708&amp;h=500`
                : item.cover.images[0] //参数iamges是个数组第一个就是参数所以为0
            }" alt="">
        </td>
        <td>${item.title}</td>
        <td>
...
document.querySelector('.next').addEventListener('click', (e) => {
  ...
  let i = totalCount / queryObj.per_page
  // console.log(i)
  if (queryObj.page < Math.ceil(i)) {
    queryObj.page++
...
// 点击编辑时，获取文章 id，跳转到发布文章页面传递文章 id 过去 跳转到publish下面的index.js的文章内容部分
